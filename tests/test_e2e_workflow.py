...
def _seed_research(client, db_path, monkeypatch):
    ...
    response = client.post(
        ...
        json={"channel_ids": ["@test"], "query": "test", "limit": 10, "generate_ideas": True},
    )
...
def _set_idea_status(client, idea_id, status):
    response = client.patch(f"/api/dashboard/ideas/{idea_id}/status", json={"status": status})
...
def test_complete_content_workflow(e2e_client, monkeypatch):
    ...
    script_response = client.post("/api/dashboard/scripts/draft", json={"idea_id": idea_id})
    ...
    production_response = client.post("/api/dashboard/production", json={"script_id": script["script_id"]})
    ...
    for status in ("ASSETS", "RENDERING"):
        response = client.patch(f"/api/dashboard/production/{production_id}/status", json={"status": status})
        assert response.status_code == 200, response.text
    ...
    response = client.patch(f"/api/dashboard/production/{production_id}/status", json={"status": "READY"})
    assert response.status_code == 200, response.text

    publish_response = client.post(
        "/api/dashboard/publishing",
        json={"production_id": production_id, "title": "Test publication", "description": "End-to-end test", "tags": ["testing", "contentos"]},
    )
    assert publish_response.status_code == 200, publish_response.text
    publish_id = publish_response.json()["publish_id"]

    response = client.patch(f"/api/dashboard/publishing/{publish_id}/status", json={"status": "SCHEDULED"})
    assert response.status_code == 200
    response = client.patch(f"/api/dashboard/publishing/{publish_id}/status", json={"status": "PUBLISHED", "external_id": "youtube-test-001"})
    assert response.status_code == 200
    assert response.json()["status"] == "PUBLISHED"

    metrics_response = client.post(
        "/api/dashboard/analytics",
        json={"external_video_id": "video-e2e-1", "views": 1000, "watch_time_minutes": 250, "average_view_duration_seconds": 15, "impressions": 5000, "click_through_rate": 8.5, "likes": 80, "comments": 12, "subscribers_gained": 25, "revenue": 4.25},
    )
...
def test_workflow_rejects_invalid_transitions_and_missing_records(e2e_client, monkeypatch):
    ...
    response = client.patch(f"/api/dashboard/ideas/{idea_id}/status", json={"status": "PUBLISHED"})
    assert response.status_code == 409
    response = client.patch("/api/dashboard/production/missing/status", json={"status": "READY"})
    assert response.status_code == 404
    response = client.patch("/api/dashboard/publishing/missing/status", json={"status": "PUBLISHED", "external_id": "x"})
    assert response.status_code == 404
    response = client.patch("/api/dashboard/automation/missing/status", json={"status": "SUCCEEDED"})
    assert response.status_code == 404

    response = client.post("/api/dashboard/analytics", json={"external_video_id": "bad", "views": -1})
    assert response.status_code == 422
    response = client.post("/api/dashboard/analytics", json={"external_video_id": "bad", "click_through_rate": 101})
    assert response.status_code == 422
    response = client.post("/api/dashboard/analytics/analyze/missing")
    assert response.status_code == 404
    response = client.post("/api/dashboard/scripts/draft", json={"idea_id": str(uuid4())})
    assert response.status_code == 404
    response = client.post("/api/dashboard/production", json={"script_id": str(uuid4())})
    assert response.status_code == 404
    response = client.post("/api/dashboard/publishing", json={"production_id": str(uuid4()), "title": "Missing production"})
    assert response.status_code == 404
