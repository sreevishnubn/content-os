...
class IdeaStatusUpdate(BaseModel): status:IdeaStatus
def require_api_token(authorization:str|None=Header(default=None))->None:
    if settings.api_token and authorization!=f"Bearer {settings.api_token}": raise HTTPException(401,"Invalid or missing API token")
def _prepare(connection,*,bootstrap=False):
    if using_postgres():initialize_postgres_schema(connection)
    else:initialize_schema(connection)
def _fetch_one(connection,sql):
    return connection.execute(text(sql)).mappings().one() if using_postgres() else connection.execute(sql).fetchone()
def _save_ideas(connection,ideas):
    if using_postgres():
        q=text("INSERT INTO content_ideas(idea_id,title,topic,audience,hook,source,why_now,monetization_angle,demand,curiosity,competition,monetization,production,overall_score,status,metadata_json,created_at) VALUES (:id,:title,:topic,:audience,:hook,:source,:why_now,:monetization,:demand,:curiosity,:competition,:monetization_score,:production,:overall,:status,:metadata,:created)")
        for i in ideas:connection.execute(q,{"id":i.idea_id,"title":i.title,"topic":i.topic,"audience":i.audience,"hook":i.hook,"source":i.source,"why_now":i.why_now,"monetization":i.monetization_angle,"demand":i.scores.demand,"curiosity":i.scores.curiosity,"competition":i.scores.competition,"monetization_score":i.scores.monetization,"production":i.scores.production,"overall":i.overall_score,"status":i.status.value,"metadata":json.dumps(i.metadata,sort_keys=True),"created":i.created_at})
    else:
        for i in ideas:connection.execute("INSERT OR REPLACE INTO content_ideas VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(i.idea_id,i.title,i.topic,i.audience,i.hook,i.source,i.why_now,i.monetization_angle,i.scores.demand,i.scores.curiosity,i.scores.competition,i.scores.monetization,i.scores.production,i.overall_score,i.status.value,json.dumps(i.metadata,sort_keys=True),i.created_at.isoformat()))
    connection.commit()
...
def health():
    ...
    try:
        c=get_connection()
        if using_postgres():c.execute(text("SELECT 1"))
        else:_prepare(c);c.execute("SELECT 1")
...
def research_youtube(request:YouTubeResearchRequest):
    ...
    c=get_connection()
    try:
        _prepare(c);ResearchRepository(c,postgres=using_postgres()).save_many(items);ideas=[score_idea(x) for x in research_items_to_ideas(items)] if request.generate_ideas and items else []
...
def dashboard_overview():
    c=get_connection()
    try:
        _prepare(c);r=_fetch_one(c,"SELECT (SELECT COUNT(*) FROM research_items) research_items,(SELECT COUNT(*) FROM content_ideas) ideas,(SELECT COUNT(*) FROM content_ideas WHERE status='APPROVED') approved,(SELECT COUNT(*) FROM content_ideas WHERE status IN ('SHORTLISTED','REVIEW')) review,(SELECT COUNT(*) FROM content_scripts) scripts,(SELECT COUNT(*) FROM production_jobs WHERE status!='FAILED') production,(SELECT COUNT(*) FROM publish_requests WHERE status='PUBLISHED') published,(SELECT COUNT(*) FROM video_metrics) analytics,(SELECT COUNT(*) FROM learning_signals) learning,(SELECT COUNT(*) FROM automation_jobs WHERE status IN ('QUEUED','RUNNING')) automation")
...
def update_idea_status(idea_id:str,request:IdeaStatusUpdate):
    c=get_connection()
    try:
        _prepare(c);r=c.execute(text("UPDATE content_ideas SET status=:status WHERE idea_id=:id"),{"status":request.status.value,"id":idea_id}) if using_postgres() else c.execute("UPDATE content_ideas SET status=? WHERE idea_id=?",(request.status.value,idea_id))
        if r.rowcount==0:raise HTTPException(404,"Idea not found")
        c.commit();return {"idea_id":idea_id,"status":request.status.value}
    finally:c.close()
@app.post("/api/admin/bootstrap",dependencies=[Depends(require_api_token)])
def bootstrap_database():
    c=get_connection()
    try:_prepare(c,bootstrap=True);return {"status":"initialized"}
    finally:c.close()
