...
from app.api.common import one, prepare_database, require_api_token
...
def health():
    ...
    try:
        ...
        raise HTTPException(
            503,
            f"Database health check failed: {type(exc).__name__}: {exc}",
        ) from exc
    finally:
        if c:
            c.close()
...
def resolve_youtube_channels(request: YouTubeResearchRequest):
    ...
    for source in request.channel_ids:
        try:
            resolved.append(
                {"input": source, "channel_id": resolve_channel_ids([source])[0]}
            )
        except Exception as exc:
            failed.append(
                {"input": source, "error": f"{type(exc).__name__}: {exc}"}
            )
...
def research_youtube(request: YouTubeResearchRequest):
    try:
        ids = resolve_channel_ids(request.channel_ids)
    except Exception as exc:
    ...
    try:
        ...
        raise HTTPException(502, f"YouTube research failed: {exc}") from exc

    c = get_connection()
    try:
        prepare_database(c)
        ResearchRepository(c, postgres=using_postgres()).save_many(items)
        ...
        return {
            ...
            "research": [x.model_dump(mode="json") for x in items],
            "ideas": [x.model_dump(mode="json") for x in ideas],
        }
    finally:
        c.close()
...
def dashboard_overview():
    ...
    try:
        ...
        r = one(
            c,
            """SELECT
                (SELECT COUNT(*) FROM research_items) research_items,
                (SELECT COUNT(*) FROM content_ideas) ideas,
                (SELECT COUNT(*) FROM content_ideas WHERE status='APPROVED') approved,
                (SELECT COUNT(*) FROM content_ideas WHERE status IN ('SHORTLISTED','REVIEW')) review,
                (SELECT COUNT(*) FROM content_scripts) scripts,
                (SELECT COUNT(*) FROM production_jobs WHERE status!='FAILED') production,
                (SELECT COUNT(*) FROM publish_requests WHERE status='PUBLISHED') published,
                (SELECT COUNT(*) FROM video_metrics) analytics,
                (SELECT COUNT(*) FROM learning_signals) learning,
                (SELECT COUNT(*) FROM automation_jobs WHERE status IN ('QUEUED','RUNNING')) automation""",
        )
...
def dashboard_ideas(limit: int = 20, status: IdeaStatus | None = None):
    ...
    try:
        ...
        sql = (
            "SELECT idea_id,title,topic,audience,hook,source,why_now,monetization_angle,"
            "demand,curiosity,competition,monetization,production,overall_score,status,"
            "metadata_json,created_at FROM content_ideas"
            + where
            + " ORDER BY overall_score DESC,created_at ASC LIMIT :limit"
        )
        p = {"limit": limit}
        if status:
        ...
        if using_postgres():
            ...
            rows = c.execute(
                sql.replace(":status", "?").replace(":limit", "?"),
                (status.value, limit),
            ).fetchall()
        else:
            rows = c.execute(sql.replace(":limit", "?"), (limit,)).fetchall()
...
def update_idea_status(idea_id: str, request: IdeaStatusUpdate):
    transitions = {
        ...
        "REVIEW": {"APPROVED", "REJECTED", "SHORTLISTED"},
        "APPROVED": {"SCRIPTING", "REJECTED"},
        "SCRIPTING": {"PRODUCTION", "APPROVED"},
        "PRODUCTION": {"REVIEW", "PUBLISHED"},
        "PUBLISHED": {"ANALYZING"},
        "ANALYZING": set(),
        "REJECTED": {"DISCOVERED"},
    }
    ...
    try:
        ...
        current = one(
            c,
            "SELECT status FROM content_ideas WHERE idea_id=:id",
            {"id": idea_id},
        )
        if not current:
            raise HTTPException(404, "Idea not found")
        new = request.status.value
        if new != current["status"] and new not in transitions.get(current["status"], set()):
            raise HTTPException(
                409,
                f"Invalid idea transition: {current['status']} -> {new}",
            )
        if using_postgres():
            c.execute(
                text("UPDATE content_ideas SET status=:status WHERE idea_id=:id"),
                {"status": new, "id": idea_id},
            )
        else:
            c.execute(
        ...
        c.commit()
        return {"idea_id": idea_id, "status": new}
    finally:
        c.close()
